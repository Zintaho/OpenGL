...
public:
	...
	Shader(const char* vsFilename, const char* fsFilename, const char* tcsFilename, const char* tesFilename) :
		vsFileName(vsFilename), fsFileName(fsFilename), tcsFileName(tcsFilename), tesFileName(tesFilename)
	{
		InitMap();
		bTessOn = true;
